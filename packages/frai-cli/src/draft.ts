...
function hasLocalAgentAuth(): boolean {
  return Boolean(process.env.ANTHROPIC_API_KEY || process.env.CLAUDE_CODE_OAUTH_TOKEN);
}

async function draftLocally(root: string): Promise<string | null> {
  try {
    const { draftGateSection } = (await import('frai-gate')) as { draftGateSection: (cwd: string) => Promise<string> };
...
async function draftHosted(root: string, files: string[], libraries: string[], projectName: string, yes: boolean): Promise<string | null> {
  const payload = {
    project: projectName,
    libraries,
  ...
  const body = (await res.json().catch(() => null)) as { section?: string; error?: string } | null;
...
function mergeIntoSpec(specPath: string, section: string): boolean {
...
export async function runDraft(options: DraftOptions = {}): Promise<number> {
  const root = options.root ?? process.cwd();
  const pkg = existsSync(path.join(root, 'package.json'))
    ? (JSON.parse(readFileSync(path.join(root, 'package.json'), 'utf8')) as { name?: string })
    : {};
  const scan = Scanners.scanCodebase({ root });
  const files: string[] = scan.aiFiles ?? [];
  ...
  for (const libs of Object.values((scan.aiLibraryMatches ?? {}) as Record<string, string[]>)) {
    for (const lib of libs ?? []) libraryNames.add(lib);
  }
