...
export function detectSiteUrl(root: string): string | null {
  ...
  if (/^https?:\/\//i.test(homepage) && !homepage.includes('github.com')) return homepage;
  ...
  for (const envFile of ['.env.production', '.env.local', '.env']) {
    ...
    if (match && !match[1].includes('localhost')) return match[1];
  }
...
export function declaredAiDependencies(root: string): string[] {
  const found = new Set<string>();
  const pkg = readJson(path.join(root, 'package.json'));
  const deps = { ...(pkg?.dependencies as object), ...(pkg?.devDependencies as object) } as Record<string, string>;
  for (const name of Object.keys(deps ?? {})) {
    if (AI_PACKAGES.includes(name) || name.startsWith('@ai-sdk/') || name.startsWith('@langchain/')) found.add(name);
  }
  for (const manifest of ['requirements.txt', 'pyproject.toml', 'Pipfile']) {
    ...
    for (const name of AI_PY_PACKAGES) {
      if (new RegExp(`(^|[\\s"'\\[])${name.replace('-', '[-_]')}\\b`, 'm').test(text)) found.add(name);
    }
  }
  return [...found];
}

function findSpec(root: string): string | null {
...
async function runGate(specPath: string): Promise<{ verdict: 'PASS' | 'WARN' | 'BLOCK'; lines: string[]; blocked: number } | null> {
  try {
    const { validateSpec } = (await import('frai-gate')) as {
      validateSpec: (markdown: string) => { verdict: string; findings: { severity: string; message: string; checkId?: string }[] };
    };
    const result = validateSpec(readFileSync(specPath, 'utf8'));
    const findings = result.findings ?? [];
    return {
      verdict: (result.verdict ?? 'BLOCK').toUpperCase() as 'PASS' | 'WARN' | 'BLOCK',
...
const SITE_CHECK_TIMEOUT_MS = 120_000;

async function checkSite(url: string): Promise<SiteCheck | { error: string }> {
  try {
    ...
    if (!res.ok) return { error: String(body.error ?? `The check failed (${res.status}).`) };
    return {
      url: String(body.url ?? url),
      verdict: body.verdict as SiteCheck['verdict'],
...
export async function runReview(options: ReviewOptions = {}): Promise<number> {
  ...
  const scan = options.siteOnly ? { aiFiles: [] as string[], totalFiles: 0, aiLibraryMatches: {} } : Scanners.scanCodebase({ root });
  const libraryCounts = new Map<string, number>();
  for (const libs of Object.values((scan.aiLibraryMatches ?? {}) as Record<string, string[]>)) {
    for (const lib of libs ?? []) libraryCounts.set(lib, (libraryCounts.get(lib) ?? 0) + 1);
  }
  ...
  if (options.json) {
    ...
    const out: string[] = ['', bold(options.siteOnly ? 'FRAI site check' : `FRAI review: ${projectName}`), ''];
    ...
    if (aiFilesFound) {
      ...
      scan.aiFiles.slice(0, 3).forEach((f: string) => out.push(dim(`    ${path.relative(root, f)}`)));
      if (scan.aiFiles.length > 3) out.push(dim(`    and ${scan.aiFiles.length - 3} more`));
    ...
    const next: string[] = [];
    if (!specPath && aiInCode) next.push('npx frai init              add the spec and the CI check');
