...
async function runQuery(prompt: string, cwd: string, maxTurns: number): Promise<string> {
  const stream = query({
    prompt,
  ...
  for await (const message of stream) {
    const anyMessage = message as { type?: string; subtype?: string; result?: string };
 ...
 */
export async function draftGateSection(cwd: string): Promise<string> {
...
<!-- RAI-GATE:END -->`;
  ...
  if (/## Responsible AI Gate/i.test(result)) return result.trim();
 ...
 */
export async function smartReview(specMarkdown: string, cwd: string): Promise<Finding[]> {
...
${specMarkdown}`;
  ...
  if (json === null) throw new Error(`Reviewer did not return findings JSON. Raw output:\n${result}`);
  const parsed = JSON.parse(json) as Array<Omit<Finding, 'source'>>;
  return parsed.map((f) => ({
