...
const logIntermediateSteps = (response: Record<string, unknown>) => {
  const steps = (response.intermediateSteps ?? []) as Array<{
...
const runSingleTurn = async (utterance: string, options: CliOptions) => {
...
const runInteractive = async (options: CliOptions) => {
  const executor = await createFraIAgentExecutor({
    model: options.model,
    verbose: options.verbose
  });
  ...
  const history: Array<AIMessage | HumanMessage> = [];
