...
npx frai-gate check RAI-SPEC.md    # BLOCK: empty answers fail (exit 1)
npx frai-gate draft                # a read-only agent drafts the gate FROM this code
npx frai-gate check RAI-SPEC.md --smart   # adversarial review of spec vs code
```
...
`TRIAGE_MODE` kill switch). `RAI-SPEC.md` is the completed spec that PASSes the
